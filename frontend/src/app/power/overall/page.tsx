...
import { fetchPowerGenerationData } from "../../../../api/power";
...
// Dynamically import ChartOne with SSR disabled
...
const PowerGeneration = () => {
  ...
  const [chartData, setChartData] = useState<{
    ...
    labels: [],
    datasets: [],
    title: "Power Generation Forecast",
    tooltipSuffix: " kWh"
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchPowerGenerationData();
        setPowerData(data);
        ...
        if (data.forecast?.length > 0) {
          ...
          // Find first non-zero actual value
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Power Generation Overview" />
